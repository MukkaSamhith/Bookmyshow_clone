...
class Details extends Component{
    ...
    getData1=async()=>{
        ...
        const response = await fetch(`http://localhost:3005/bollywood/${tes1}`)
    ...
    render(){
        const {x} = this.state
        const {movieUrl,movieName,id,directorName,producerName,actors,budget} = x
    return(
        <div className="sam">
            ...
            <p>Actors Name:{actors}</p>
            <p>Budget:{budget} Cr</p>
            <hr className="sam2"/>
            <Link to ={`/bollywood/seats/${id}`}>
