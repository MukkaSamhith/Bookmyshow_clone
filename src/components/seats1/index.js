...
class Seats extends Component{
    ...
    getSeats=async()=>{
        const {tes2} = this.props
        const response = await fetch(`http://localhost:3005/bollywood/seats/${tes2}`)
        ...
        const response1 = await fetch(`http://localhost:3005/bollywood/${tes2}`)
    ...
    onMove=async()=>{
        ...
        const response = await fetch(`http://localhost:3005/bollywood/seats/${tes2}`,options)
    }
    ...
    render(){
        ...
        let is=null
        if(k===true){
            is = <h1 className="ff">Booking Details has sent to your mailID</h1>
        }
        return(
