...
import MovieDetails2 from '../MovieDetails2/index.js';
...
class View2 extends Component{
  ...
  getData = async()=>{
    const response = await fetch("http://localhost:3005/tollywood/")
  ...
  sendOf=async()=>{
    ...
    console.log(options)
    const response = await fetch("http://localhost:3005/tollywood/",options)
    ...
    <div className='pra'>
        <div className='pra1'>
            ...
            <div className='displayMovie'>
                {x.map((each)=>(
                <MovieDetails2 key={each.id} tes={each}/>
                ))}
              </div>
            </div>
        </div>
